defmodule OliWeb.Curriculum.Rollup do
  ...
  def page_updated(
    ...
    page_activity_map =
      Map.put(struct.page_activity_map, revision.resource_id, MapSet.to_list(current))
    ...
    {activity_map, objective_map} =
      case added do
        ...
        _ ->
          ...
          objective_map =
            Map.merge(
              struct.objective_map,
              build_objective_map(project_slug, partial_activity_map)
            )
  ...
  def activity_updated(%__MODULE__{} = struct, %Revision{} = revision, project_slug) do
    ...
    if MapSet.equal?(old_objectives, updated_objectives) do
      ...
      objective_map =
        Map.merge(struct.objective_map, build_objective_map(project_slug, partial_activity_map))
  ...
  defp build_activity_map(project_slug, page_to_activities_map) do
    all_activities =
      Enum.map(page_to_activities_map, fn {_, activity_ids} -> activity_ids end)
      |> List.flatten()
